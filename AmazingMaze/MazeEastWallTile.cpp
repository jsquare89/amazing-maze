...
namespace AmazingMaze
{
    namespace detail
    {
        ...
        {          
            glBegin(GL_POLYGON);                
                ...
                glTexCoord2f(0, 1);
                glVertex3fv(east_vertices[a]);
                glNormal3fv(normals[b]);
                glTexCoord2f(0, 0);
                glVertex3fv(east_vertices[b]);
                glNormal3fv(normals[c]);
                glTexCoord2f(1, 0);
                glVertex3fv(east_vertices[c]);
                glNormal3fv(normals[d]);
                glTexCoord2f(1, 1);
                glVertex3fv(east_vertices[d]);
            glEnd();
        }
    } // namespace detail

    CMazeEastWallTile::CMazeEastWallTile(void) : Egl::CModel(),
                                                 m_nPrecompiledId(2)
    {   
        // TODO
    ...
    // virtual
    CMazeEastWallTile::~CMazeEastWallTile(void) throw()
    {        
    }

    // virtual 
    const float 
    CMazeEastWallTile::GetWidth() const
    {
        float nWidth = 1;
    ...
    // virtual 
    const float 
    CMazeEastWallTile::GetHeight() const
    {
        // Calculate height
        float nHeight = 1;
